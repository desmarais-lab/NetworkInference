...
# Generate df of possible edges + oon diffusions

all_nodes = letters[1:5]
cascade_nodes = letters[1:3]
cascade_times = 1:3
names(cascade_times) = cascade_nodes
censoring_time = 7

## For one casacade

# All possible edges from the cascade itself

# expand grid on node at index with all nodes of higher index (later in cascade)
# O(n^2)
eg_following = function(index, nodes) {
    if(index == length(nodes)) return(NULL)
    return(expand.grid(nodes[index], nodes[(index + 1):length(nodes)]))
}
grids = lapply(1:length(cascade_nodes), eg_following, cascade_nodes)
possible_edges = do.call(rbind, grids)
possible_edges$not_censored = TRUE

# now with all out of cascade nodes
excluded_nodes = all_nodes[!(all_nodes %in% cascade_nodes)] 
censored_edges = expand.grid(cascade_nodes, excluded_nodes)
censored_edges$not_censored = FALSE

library(survival)
# Combine
edge_df = rbind(possible_edges, censored_edges)
edge_df[, 1] = as.character(edge_df[, 1])
edge_df[, 2] = as.character(edge_df[, 2])
colnames(edge_df) = c('origin', 'destination', 'not_censored')

# Fill in times
edge_df$diff_time = cascade_times[edge_df$destination] - 
                    cascade_times[edge_df$origin]
edge_df$diff_time = ifelse(edge_df$not_censored, edge_df$diff_time, 
                           censoring_time - cascade_times[edge_df$origin])
outcome = Surv(time = edge_df$diff_time, event = edge_df$not_censored, 
               type = 'right')

mod = survreg(outcome ~ 1)
