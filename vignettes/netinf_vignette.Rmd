...
Then fit the model:
```{r}
result <- netinf(cascades, trans_mod = "exponential", params = 1, n_edges = 5)
```
...
`trans_mod` specifies the transition model, or the parametric model according to which the times between diffusion events are distributed. See the `netinf` documentation (`?netinf`) for the available distributions. For this example we choose the exponential density. 

`params` fixes the paramters of the distribution specified in `trans_mod`. In this case it's the rate parameter of the exponential density.
