...
In this case the data is stored in a matrix in wide format. Each row corresponds to a state and each column corresponds to a policy. The cell entries indicate the year a state adopted a policy (or `NA` in case the policy was never adopted). Let's look at the upper left corner of this matrix:
...
```{r}
policy_cascades <- as_cascade_wide(policies)
```

In this case we used the function `as_cascade_wide`. If your data is in long format
(i.e. each row corresponds to an event rather than a node or unit) you can convert
it using the function `as_cascade_long`.
...
In the original implementation the parameters of that distribution are a tuning parameter, i.e. they have to be chosen manually. Furthermore, the number of edges to infer has to be fixed too. If you want to run `netinf` in this classic way you can do so by specifiying the respective parameters (see below for an explanation of the `max_iter` option):

```{r}
results <- netinf(policy_cascades, trans_mod = "exponential", n_edges = 100, 
                  params = 0.5, max_iter = 1, quiet = TRUE)
```
...
We see that with a fixed lambda of 0.5 and a p-value cut-off of 0.1 the algorithm inferred 471 edges.

#### Selecting the parameters of the diffusion model

The diffusion model parameters can be selected automatically. Setting the `params` argument to `NULL` (default value) makes the `netinf` function infer parameters iteratively using approximate profile maximum likelihood. The parameters are initialized at the midpoint between the MLE of the minimum diffusion times and the MLE of the maximum diffusion times, across all cascades. Edges are then inferred until either the p-value cut-off or a manually specified number of edges (`n_edges`) is reached. The parameters are then updated using the diffusion times that result when using the inferred edges for the diffusion tree of each cascades. This procedure is repeated until either the network converges (i.e. all inferred edges are the same) or `max_iter` iterations are reached (if we want to just initialize the parameters automatically but not update them we can just set `max_iter = 1` and `params = NULL`). 

```{r, cache=TRUE}
results <- netinf(policy_cascades, trans_mod = "exponential", n_edges = 0.1, 
                  max_iter = 10, quiet = TRUE)
```

The final parameter values and number of iterations used can be acessed from the results object via the attributes `diffusion_model_parameters` and `n_iterations`:

```{r}
attr(results, 'diffusion_model_parameters')
attr(results, 'n_iterations')
```
